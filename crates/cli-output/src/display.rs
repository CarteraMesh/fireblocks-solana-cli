use {
    ...
    solana_sdk::{
        clock::UnixTimestamp,
        hash::Hash,
        message::{compiled_instruction::CompiledInstruction, v0::MessageAddressTableLookup},
        pubkey::Pubkey,
        signature::Signature,
...
fn write_instruction<'a, W: io::Write>(
    ...
    if let AccountKeyType::Known(program_pubkey) = program_pubkey {
        if program_pubkey == &solana_vote_interface::program::id() {
            if let Ok(vote_instruction) =
                limited_deserialize::<solana_vote_interface::instruction::VoteInstruction>(
                    &instruction.data,
